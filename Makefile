...
test-integration:
	...
	TEST_DATABASE_URL="postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@127.0.0.1:$(TEST_POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable" go test -count=1 -p=1 -tags=integration ./internal/database ./internal/user ./internal/equipment
